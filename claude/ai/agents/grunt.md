---
name: grunt
description: Delegates mechanical, well-specified work to DeepSeek via OpenCode (with fallbacks). Use for codebase surveys, log reading, renames across files, boilerplate, test scaffolding, first-pass summaries. Not for design decisions, security-sensitive code, or anything ambiguous.
...
3. If it exits 75, every cheap lane is limited: report that and return the brief so the caller can decide.
