...
Codex reviews it; if Codex is at its limit the script falls back on its own: free OpenCode models first (OPENCODE_FREE_MODELS — they keep working while OpenCode Go is at its limit), then the Go models. When the `REVIEWER:` line says `free`, the review cost nothing — name the model in the report. When the `REVIEWER:` line says `downgraded`, the weekly-budget rule ran the review on the cheap Codex tier — name that in the report. If it exits with code 75, no external reviewer is reachable: delegate the same review to the `reviewer` agent and mark that review as same-vendor in the report.
